...
delta_bacteria = 0
bacteria_add_time = (540,)
bacteria_to_add = 100
exposure = "oscillation"
simulate_desync = True
desync_factor = 20 # percent
desync_sampling = 3000
...
with open("Circadian simulation.csv", "w") as file:
    ...
    while current_step < steps:
        ...
        if exposure == "stimulation":
            if current_step in bacteria_add_time:
                bacteria += bacteria_to_add

        # bacterial exposure oscillates
        elif exposure == "oscillation":
            if current_step >= min(bacteria_add_time):
                bacteria = 6 + 5 * math.cos(((current_step / 10) - 60) * 2 * math.pi / 24)
